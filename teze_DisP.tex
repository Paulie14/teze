...
které obsahují {ele\-men\-ty} různých dimenzí. V takových modelech se často potýkáme s~velkou nepřesností aproximace
lokálních jevů, když celková zájmová oblast je rozsáhlá. 
Použití některé z technik adaptivního zjemňování sítě vede na systémy s enormním počtem neznámých, obzvláště ve 3D.
...
on the problematics. Modelling on meshes of combined dimension is addressed in Section \ref{sec:soa_model_combined},
survey on XFEM is presented in Section \ref{sec:soa_xfem} and works regarding the usage of XFEM on
...
The finished and ongoing work is presented in chapter \ref{chap:results}. The Section \ref{sec:model_aquifer} 
covers a well-aquifer model of groundwater flow with XFEM resolving a~point singularity in 2D.
The Section \ref{sec:elements_intersections} introduces a new approach for inspecting simplicial elements intersections
...
media with combined mesh dimensions, theresore we shall often use the related terminology.
...
\caption{
    \label{fig:multi-dim}
    Scheme of a problem with domains of multiple dimensions, (used from \cite{flow123d_doc_2015}).
}
\end{figure}
...
The elements in \emph{a compatible mesh} must satify the following compatibility conditions
\begin{equation}
...
Creation of a compatible mesh can be challenging, due to the compatibility conditions and also
element quality requirement. Therefore the aim is to to avoid these meshes.
...
The main feature of this method is the extension of a space of basis polynomial shape functions of finite element
...
We are intersected in meshbased methods with extrinsic enrichment and we shall not discuss any meshfree alternatives.

An overview article~\cite{fries_xfem_overview_2010} by Fries and Belytschko concludes the history and early development
...
The most frequent application of the XFEM is in mechanics, where discontinous functions (Heaviside function, signum)
...
become nearly linearly dependent. It was thought that the SGFEM is the answer to that problem but it has been
reported that it is still not a general solution.

Another problem is the stability of the discretisation. In contrast to mechanics, the PDEs describing flow
need to satify a strict discrete inf-sup condition (inequation 3.14, p. 75 in \cite{brezzi_mixed_1991} for mixed formulation).
...
is a simulator of underground water flow, solute and heat transport in fractured porous media.
\begin{center}
the software web page: \url{http://flow123d.github.io/}
\end{center}
It has been developed at the Technical University of Liberec approximately since 2007.
The main feature of the software is computation on complex compatible meshes of combined dimensions, described in
...
Developement of the software is an important practical part of the thesis. The aim is to implement support 
...
The aims of the thesis can be summarized in two major points
\begin{itemize}
%   \item Research of current works concerning modelling on meshes of combined dimensions, XFEM methods, Mixed-hybrid 
%         method (in particular for the Laplace equation) and their combination.
...
\section{Model}
The model with mixed-hybrid formulation as in \cite{brezina_mixed-hybrid_2010, sistek_bddc_2015} shall be followed.
The enrichment shall be chosen and incorporated in the formulation. The inf-sup condition shall be investigated
and some constraints on the choice of the enrichment functions shall be defined. 

\section{Implementation}
The following upgrades and changes in the software Flow123d are to be considered.
...
A class that handles the degrees of freedom must be then updated to include the enriched degrees of freedom
...
Finally, a~reasonable output postprocessing should be developed, so that the XFEM solution (piecewise non-polynomial solution)
...
In addtition to article \cite{exner_2016} the model formulation was tuned with the averaged term on the well
